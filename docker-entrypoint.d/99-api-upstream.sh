#!/bin/sh
set -e
# 1) API_UPSTREAM explÃƒÆ’Ã‚Â­cito (ej. api-backend:3001 desde compose raÃƒÆ’Ã‚Â­z)
# 2) Si no: host.docker.internal + HOST_API_PORT (mismo nÃƒÆ’Ã‚Âºmero que PORT del API en npm run dev)
